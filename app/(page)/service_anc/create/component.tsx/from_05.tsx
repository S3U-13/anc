  ...
  return (
    <div className="grid grid-cols-4 overflow-y-scroll max-h-[calc(90vh-300px)]">
      ...
      >
        {data
          .filter(
            (ref_in, index) => ref_in.choice_type_id === 15 && index !== 1
          )
          .map((ref_in) => (
            <div
              key={ref_in.id}
      ...
      >
        {data
          .filter(
            (ref_out, index) =>
              ref_out.choice_type_id === 15 && [1, 3].includes(index)
          )
          .map((ref_out) => (
            <div
              key={ref_out.id}
            ...
            >
              ...
              {String(ref_out.id) === 41 && field.ref_out_id === 41 && (
                <Input size="sm" label="รพช/รพสต" type="text" />
              )}
            </div>
          ))}
      </CheckboxGroup> */}
      {data
        .filter((receive_in) => receive_in.choice_type_id === 16)
        .map((receive_in) => (
          <div key={receive_in.id}>
            <p>{receive_in.choice_name}</p>
          </div>
        ))}
    </div>
  );
