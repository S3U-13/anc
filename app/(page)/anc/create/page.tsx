...
export default function page({ openModal, closeModal }) {
  const { field, handleSearchHnWife, hnInputWife, setHnInputWife, pat, formatAddress } = useHook();
  return (
                ...
                <Tab key="wife" title="ส่วนของภรรยา">
                  <div className='overflow-y-scroll max-h-[calc(90vh-300px)]'>
                    ...
                    <div className='grid grid-cols-2 gap-[10px] mt-[15px]'>
                      <Input size='sm' label="HN" name='hn_wife' value={field.hn_wife} type="text" />
                      <Input size='sm' label="ชื่อ" value={`${pat?.prename || ""} ${pat?.firstname || ""} ${pat?.lastname || ""}`} type="text" readOnly/>
                      <Input size='sm' label="อายุ" type="text" readOnly/>
                      ...
                      <Input size='sm' label="เบอร์โทรศัพท์" type="text" />
