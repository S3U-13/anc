...
export default function page({ openModal, closeModal }) {
  const { field, handleSearchHnWife, hnInputWife, setHnInputWife, handleSearchHnHusband, hnInputHusband, setHnInputHusband, pat, patHusband, formatAddress, formatName, formatNameHusband, vitals, bmi, editVitalsign, handleEditChange, calculateAge, handleChange, } = useHook();
  return (
            ...
            <ModalBody>
              <Tabs color='default' classNames={{ tabContent: "group-data-[selected=true]:bg-[#ffffff] p-1 px-6 rounded-lg", tabList: 'mx-auto' }} aria-label="Dynamic tabs">
                <Tab key="wife" title="ส่วนของภรรยา">
                  <div className='overflow-y-scroll max-h-[calc(90vh-300px)]'>
                    ...
                    <div className='grid grid-cols-2 gap-[10px] mt-[15px]'>
                      <Input size='sm' label="HN" value={field.hn_wife} type="text" />
                ...
                <Tab key="husband" title="ส่วนของสามี">
                  ...
                  <div className='grid grid-cols-2 gap-[10px] mt-[15px]'>
                    <Input label="HN สามี" value={field.hn_husband} type="text" />
                    <Input label="ชื่อ สามี" value={formatName(patHusband)} type="text" />
